...
var debug			= require('debug')('i18nc-key-combo:test_base');
...
describe('#keyCombo', function()
{
	...
	describe('#combo', function()
	{
		...
		function _realCheck(codeStr, eqlInfo, mode)
		{
			...
			{
				var arr = keyComboTest._plusBinaryExpressionAst2arrWidthClear(data, options);
				expect(_item2val(arr)).to.eql(eqlInfo.combo);
			});

			it('#combo', function()
			{
				var newAst = keyCombo.combo(data, options);
	...
	describe('#revert', function()
	{
		...
		function _genIt(name, dealWordsArr, eqlObj)
		{
			it('#'+name, function()
			{
				var result = keyCombo.revert(dealWordsArr, comboAsts, options);
